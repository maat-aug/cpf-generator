// GradientWaves — ported from React Bits (https://reactbits.dev) to vanilla JS/WebGL.
// Renders an animated raymarched wave field via `ogl` into a container element.
...
const detailToSteps = (detail: string | undefined): number => {
...
const DEFAULTS: Required<GradientWavesOptions> = {
  horizonColor: '#5227FF',
  waveColor: '#FF9FFC',
  crestColor: '#FFFFFF',
  speed: 0.4,
...
export function mountGradientWaves(container: HTMLElement, options: GradientWavesOptions = {}): () => void {
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uHorizonColor: { value: new Float32Array(hexToRgb(opts.horizonColor)) },
      uWaveColor: { value: new Float32Array(hexToRgb(opts.waveColor)) },
      uCrestColor: { value: new Float32Array(hexToRgb(opts.crestColor)) },
    },
  });
